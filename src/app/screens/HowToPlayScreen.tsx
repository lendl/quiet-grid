      ...
      {rows.map((row, ri) => (
        <View key={ri} style={{ flexDirection: 'row', gap: 3 }}>
          {row.map((cell, ci) => {
            ...
            const val = cell === 'a0' ? 0 : cell === 'a1' ? 1 : cell;
            return (
              <View
                key={ci}
                style={{
                  ...
                  backgroundColor: isAnswer ? theme.primary : val !== null ? theme.filledBackground : theme.surfaceElevated,
                  borderWidth: 1,
              ...
              >
                <Text style={{
                  ...
                  color: isAnswer ? '#fff' : val !== null ? theme.filled : theme.textMuted,
                }}>
                  {val !== null ? String(val) : ''}
                </Text>
              </View>
    ...
    <ScrollView contentContainerStyle={s.scroll}>
        ...
        {howToPlay.tips.map(tip => (
          <View key={tip.key} style={s.tipCard}>
            ...
            {tip.example && <MiniGrid rows={tip.example} />}
          </View>
        ))}
