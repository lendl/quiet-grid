...
@Composable
fun FlowFreePlayScreen(
    difficulty: Difficulty,
    resume: Boolean,
    onBack: () -> Unit,
    onFinished: (FlowFreeResult) -> Unit,
) {
    val viewModel = hiltViewModel<FlowFreePlayViewModel, FlowFreePlayViewModel.Factory>(
    ...
    Column(Modifier.fillMaxSize().padding(16.dp)) {
        ...
        ElapsedTimerText(viewModel.elapsedSeconds.toInt(), modifier = Modifier.padding(top = 4.dp, bottom = 12.dp))

        PuzzleBoardContainer(visible = session != null, playFresh = !resume, flashTrigger = 0) {
            if (session != null) {
