    ...
    Column(Modifier.fillMaxSize().padding(16.dp)) {
        ...
        if (session != null) {
            Row(Modifier.fillMaxWidth().padding(top = 4.dp, bottom = 12.dp), horizontalArrangement = androidx.compose.foundation.layout.Arrangement.SpaceBetween) {
                ...
                Text(
                    formatElapsed(maxOf(0, session.secondsRemaining.toInt())),
                    style = MaterialTheme.typography.bodyMedium,
                    color = if (session.secondsRemaining <= 10) MaterialTheme.colorScheme.error else MaterialTheme.colorScheme.onSurface,
                )
            }
        }
