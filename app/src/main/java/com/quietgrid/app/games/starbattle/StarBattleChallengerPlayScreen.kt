...
@Composable
fun StarBattleChallengerPlayScreen(
    onFinished: (StarBattleChallengerResult) -> Unit,
) {
    val viewModel = hiltViewModel<StarBattleChallengerViewModel>()
    ...
    Column(Modifier.fillMaxSize().padding(16.dp)) {
        Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
            ...
            Text(
                stringResource(R.string.starbattle_challenger_label),
        ...
        if (session != null) {
            Row(Modifier.fillMaxWidth().padding(top = 4.dp, bottom = 12.dp), horizontalArrangement = Arrangement.SpaceBetween) {
                Text(
                    "${stringResource(R.string.starbattle_challenger_tier_label)}: ${stringResource(starBattleDifficultyLabelRes(session.tier))}",
                    style = MaterialTheme.typography.bodyMedium,
                )
                Text(
                    "${stringResource(R.string.starbattle_challenger_solved_label)}: ${session.puzzlesSolved}",
                    style = MaterialTheme.typography.bodyMedium,
                )
