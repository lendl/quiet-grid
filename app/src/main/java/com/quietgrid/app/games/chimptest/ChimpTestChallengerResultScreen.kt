    ...
    Column(Modifier.fillMaxSize().padding(16.dp)) {
        Box(Modifier.weight(1f).fillMaxWidth(), contentAlignment = Alignment.Center) {
            Column(horizontalAlignment = Alignment.CenterHorizontally) {
                ...
                if (isNewHighScore) {
                    Text(
                        stringResource(R.string.chimp_challenger_result_new_high_score),
                ...
                Row(
                    ...
                    ResultMetaItem(stringResource(R.string.chimp_challenger_result_puzzles_solved), puzzlesSolved.toString())
                    ResultMetaItem(stringResource(R.string.chimp_challenger_result_tier_reached), stringResource(chimpDifficultyLabelRes(tierReached)))
                    ResultMetaItem(stringResource(R.string.chimp_challenger_result_score), score.toString())
                }

                Button(onClick = onPlayAgain, modifier = Modifier.fillMaxWidth().padding(top = 24.dp)) {
                    Text(stringResource(R.string.chimp_challenger_play_again))
                }
                ...
                Row(
                    ...
                    TextButton(onClick = onBackToPuzzles) {
                        Text(stringResource(R.string.chimp_challenger_back_to_puzzles), color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                    TextButton(onClick = onTryAnotherGame) {
                        Text(stringResource(R.string.chimp_challenger_try_another_game), color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                }
            }
        }
