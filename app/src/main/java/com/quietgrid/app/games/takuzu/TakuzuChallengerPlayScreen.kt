...
fun TakuzuChallengerPlayScreen(
    ...
    Column(Modifier.fillMaxSize().padding(16.dp)) {
        Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
            ...
            Text(
                stringResource(R.string.takuzu_challenger_label),
                style = MaterialTheme.typography.titleMedium,
                modifier = Modifier.padding(start = 12.dp).weight(1f),
            )
            if (session != null) {
                Row(Modifier.padding(end = 8.dp)) {
                    repeat(TAKUZU_CHALLENGER_STARTING_LIVES) { index ->
                        ChallengerHeartIcon(
        ...
        if (session != null) {
            Row(Modifier.fillMaxWidth().padding(top = 4.dp, bottom = 12.dp), horizontalArrangement = Arrangement.SpaceBetween) {
                Text(
                    "${stringResource(R.string.takuzu_challenger_tier_label)}: ${stringResource(takuzuDifficultyLabelRes(session.tier))}",
                    style = MaterialTheme.typography.bodyMedium,
                )
                Text(
                    "${stringResource(R.string.takuzu_challenger_solved_label)}: ${session.puzzlesSolved}",
                    style = MaterialTheme.typography.bodyMedium,
                )
        ...
        PuzzleBoardContainer(
            ...
            session?.let { current ->
                TakuzuBoard(
                    ...
                    isGiven = current.puzzleSession.isGiven,
