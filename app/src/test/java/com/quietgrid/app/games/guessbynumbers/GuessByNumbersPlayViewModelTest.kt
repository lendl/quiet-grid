...
class GuessByNumbersPlayViewModelTest {

    @get:Rule
    val mainDispatcherRule = MainDispatcherRule()

    private val puzzleEntry = WordGuessPuzzleEntry(
        id = "test-guessbynumbers",
        locale = "en",
    ...
    private fun newViewModel(sessionStore: FakeSessionStore = FakeSessionStore(), statsStore: FakeStatsStore = FakeStatsStore()): GuessByNumbersPlayViewModel {
        val settingsRepository = mockk<SettingsRepository>(relaxed = true)
        every { settingsRepository.settings } returns MutableStateFlow(AppSettings())
        return GuessByNumbersPlayViewModel(
    ...
    fun `submitting the target word wins the puzzle`() {
        ...
        val results = mutableListOf<GuessByNumbersResult>()
